...
return view.extend({
	...
	render: function(result) {
		...
		m = new form.Map('adblock', 'Arca Parental Controls');
		...
		*/
		pollData: poll.add(function() {
			return L.resolveDefault(fs.read_direct('/tmp/adb_runtime.json'), 'null').then(function(res) {
				...
				if (status && info) {
					status.textContent = ('Arca Parental Controls');
				...
				var src_array = [];
				if (sources && info) {
					sources.textContent = 'Advanced Arca Cyber Protection';
				}
				var backend = document.getElementById('backend');
				if (backend && info) {
					backend.textContent = 'Arca Parental Controls V1';
				}
				var utils = document.getElementById('utils');
				if (utils && info) {
					utils.textContent = '/Arca Parental Controls';
				}
				...
				if (dirs && info) {
					dirs.textContent = '/Arca Security';
				}
		...
		*/
		o = s.taboption('general', form.Flag, 'adb_enabled', _('Enabled'), _('Enable the Parental Control service.'));
		o.rmempty = false;

		o = s.taboption('general', widgets.NetworkSelect, 'adb_trigger', _('Startup Trigger Interface'), _('List of available network interfaces to trigger the Arca Parental Controls start. \
		...
		o = s.taboption('general', form.Flag, 'adb_report', _('DNS Report'), _('Gather DNS related network traffic and provide a DNS Report on demand'));
		o.rmempty = false;
		...
		*/
		...
		o = s.taboption('additional', form.Flag, 'adb_nice', _('Low Priority Service'), _('Reduce the priority of the Parental Controls  background processing to take fewer resources from the system. \
		...
		o = s.taboption('additional', form.Value, 'adb_tmpbase', _('Base Temp Directory'), _('Base Temp Directory for all Parental Controls related runtime operations, \
		...
		*/
		...
		o = s.taboption('adv_dns', form.Value, 'adb_dnsdir', _('DNS Directory'), _('Target directory for the generated blocklist'));
		o.placeholder = '/tmp';
		o.rmempty = true;
		...
		*/
		o = s.taboption('adv_email', form.Value, 'adb_mailsender', _('E-Mail Sender Address'), _('Sender address for Arca Parental Control  notification E-Mails.'));
		o.placeholder = 'no-reply@adblock';
		o.rmempty = true;

		o = s.taboption('adv_email', form.Value, 'adb_mailtopic', _('E-Mail Topic'), _('Topic for Arca Parental Control  notification E-Mails.'));
		o.placeholder = 'adblock notification';
		o.rmempty = true;

		o = s.taboption('adv_email', form.Value, 'adb_mailprofile', _('E-Mail Profile'), _('Profile used by \'msmtp\' for  notification E-Mails.'));
